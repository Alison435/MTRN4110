  ...
  switch(system_mode) {
    ...
    case(MODE_SPEEDRUN):
      ...
      while(!goal) 
      {
        ...
        //(not sure if this is still needed, does it affect swapdir?)
        compassd = compass(); //update compass 
        ...
        // Need to put it into a loop to iterate through pathTwo array
        ...
        for (int j = 0; j < sizeof(pathTwo); j++)
        {
         ...
         // Are these always in the robot's local coordinate frame?
         // NORTH 0
         // EAST 1
         // SOUTH 2
         // WEST 3          

          if (nextdir == 6) continue;

          // to the robot, north or south makes no difference?
          if (pathTwo[j] == NORTH || pathTwo[j] == SOUTH)
          {
             resetAllEncoders();
             delay(150);
             robotForward(SPEEDRUN_DISTANCE,SPEEDRUN_SPEED); 
             //go forward one cell (might need to reduce 
             //the encoder straight distance at 35% PWM)
             delay(50);
             resetAllEncoders();
          }
          else
